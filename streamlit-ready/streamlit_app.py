...
def resolve_embedded_frontend_url() -> Optional[str]:
    ...
    # Priority: Streamlit secret > env var.
    # Exact embed mode is intentionally opt-in only.
...
def render_exact_embedded_ui() -> bool:
    ...
    st.set_page_config(page_title="Steak Cook Time Estimator", layout="wide")
    st.markdown(
        """
        <style>
    ...
    st.caption(
        ...
        "Set `frontend_url` in Streamlit secrets (or `STEAK_FRONTEND_URL`) to change source."
    )
    return True
...
def inject_ui_style() -> None:
    st.markdown(
        """
        <style>
        @import url("https://fonts.googleapis.com/css2?family=Barlow+Semi+Condensed:wght@500;600;700;800&family=Oswald:wght@500;600;700&display=swap");

        :root {
          --navy-950: #120f0e;
          --navy-900: #231916;
          --hero-ink: #f7fbff;
          --hero-sub: rgba(247, 239, 227, 0.95);
          --card: #f6f2ea;
          --card-soft: #fbf7f0;
          --ink: #221814;
          --muted: #5a473d;
        ...
        [data-testid="stAppViewContainer"] {
          background:
            radial-gradient(circle at 20% 0%, rgba(160, 63, 50, 0.22), transparent 40%),
            radial-gradient(circle at 95% 10%, rgba(122, 52, 42, 0.2), transparent 42%),
            linear-gradient(165deg, var(--navy-950) 0%, var(--navy-900) 62%, #2d211d 100%);
        }
        ...
        .hero-panel {
          border: 1px solid rgba(222, 199, 174, 0.3);
          border-radius: 16px;
          padding: 18px 20px;
          margin-bottom: 16px;
          background: linear-gradient(160deg, #241917, #3a2a26);
          box-shadow: 0 22px 48px rgba(3, 10, 25, 0.34);
        }
        ...
        [data-testid="stVerticalBlockBorderWrapper"] {
          ...
          box-shadow: 0 22px 48px rgba(3, 10, 25, 0.24) !important;
        }
...
def render_hero() -> None:
    st.markdown(
        ...
        <div class="hero-panel">
          <p class="hero-kicker">Fire Up The Oven</p>
          <h1 class="hero-title">How Long Should I Cook My Steak? We Tell You.</h1>
          <p class="hero-sub">Choose your cut, doneness, and oven setup to get a practical time range.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def render_result_card(result: Dict[str, Any], oven_temp_f: int, oven_mode: str) -> None:
    ...
    st.markdown(
        ...
        <div class="quick-summary"><b>Quick summary:</b> Roast on a wire rack for the estimated oven time, pull at 125F, rest 5 minutes, then sear 90 seconds per side in a very hot skillet with tallow or high smoke-point oil.</div>
        <p class="disclaimer">Even if you set your oven to 250F, actual internal oven temperature can vary by oven. Keeping a thermometer handy to double-check is always helpful.</p>
        """,
        unsafe_allow_html=True,
    )
...
def main() -> None:
    ...
    with left_col:
        with st.container(border=True):
            ...
            current_doneness_id = (
                st.session_state.doneness_id
                if st.session_state.doneness_id in doneness_ids
                else get_default_doneness_id(starter_data)
            )
            current_doneness_idx = doneness_ids.index(current_doneness_id)
            selected_doneness_idx = st.select_slider(
                "Doneness level",
                options=list(range(len(doneness_targets))),
                value=current_doneness_idx,
                format_func=lambda i: doneness_targets[i]["doneness_id"].replace("_", " ").title(),
                label_visibility="collapsed",
            )
            selected_doneness = doneness_targets[selected_doneness_idx]
            st.session_state.doneness_id = selected_doneness["doneness_id"]
            st.markdown(
                f'<p class="field-note">Target {selected_doneness["target_temp_f"]}F / Pull {selected_doneness["pull_temp_f"]}F</p>',
                unsafe_allow_html=True,
            )
            ...
            with c1:
                ...
                thickness_in = st.slider(
                    "Thickness (in)",
                    min_value=0.5,
                    max_value=2.5,
                    value=float(st.session_state.thickness_in),
                    step=0.25,
                    label_visibility="collapsed",
                )
                st.session_state.thickness_in = float(thickness_in)
                st.markdown(
                    f'<p class="field-note">{thickness_in:.2f} in</p>',
                    unsafe_allow_html=True,
                )
            ...
            with c2:
                ...
                weight_slider = st.slider(
                    ...
                    min_value=4.0,
                    max_value=64.0,
                    value=float(clamp(float(st.session_state.weight_slider), 4, 64)),
                    step=0.5,
                    label_visibility="collapsed",
                )
                st.session_state.weight_slider = float(weight_slider)
                if not st.session_state.weight_custom and abs(weight_slider - auto_weight) > 0.01:
                    st.session_state.weight_custom = True

                st.markdown(
                    f'<p class="field-note">{weight_slider:.1f} oz</p>',
                    unsafe_allow_html=True,
                )
                if st.session_state.weight_custom:
                    if st.button("Use default", key="reset_weight_default", use_container_width=True):
                        st.session_state.weight_custom = False
                        st.session_state.weight_slider = float(round(auto_weight * 2) / 2)
                        st.rerun()
                else:
                    st.caption(f"Default: {auto_weight:.1f} oz")
            ...
            oven_temp_f = int(
                st.slider(
                    ...
                    min_value=250,
                    max_value=500,
                    value=int(st.session_state.oven_temp_f),
                    step=25,
                    label_visibility="collapsed",
                )
            )
            st.session_state.oven_temp_f = oven_temp_f
            st.markdown(
                '<div class="major-ticks"><span>250</span><span>300</span><span>350</span><span>400</span><span>450</span><span>500</span></div>',
                unsafe_allow_html=True,
            )
            ...
            with c3:
                ...
                st.markdown('<p class="field-note">Reverse sear (oven first, sear after)</p>', unsafe_allow_html=True)
            ...
            start_idx = 0 if st.session_state.start_temp_assumption == "fridge" else 1
            st.session_state.start_temp_assumption = st.radio(
                "Starting temp",
                options=["fridge", "tempered"],
                index=start_idx,
                format_func=lambda x: "From fridge" if x == "fridge" else "Tempered",
                horizontal=True,
                label_visibility="collapsed",
            )
